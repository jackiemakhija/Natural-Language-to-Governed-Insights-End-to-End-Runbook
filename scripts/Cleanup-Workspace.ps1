...
Write-Host "Workspace cleanup - Natural-Language-to-Governed-Insights" -ForegroundColor Cyan
...
function Get-Matches([string]$pattern){ try { Get-ChildItem -Path $pattern -Recurse -Force -ErrorAction SilentlyContinue } catch { @() } }
